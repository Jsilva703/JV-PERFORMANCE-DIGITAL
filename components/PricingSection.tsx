...
const plans: Plan[] = [
    {
        ...
        description: 'Para validar uma ideia, criar o primeiro site ou para projetos com orçamento inicial limitado.',
        price: 'R$ 450',
        ...
        features: [
            ...
            'Hospedagem gratuita no GitHub Pages (com endereço github.io)',
        ],
    ...
    },
    {
        ...
        description: 'Para negócios que buscam credibilidade, marca própria e a melhor performance digital.',
        price: 'R$ 600',
        priceDetails: 'Taxa de Desenvolvimento',
        features: [
            'Tudo do Plano de Lançamento, e mais:',
    ...
    },
    {
        ...
        description: 'Precisa de e-commerce, área de membros, ou sistemas complexos? Em breve esta solução completa estará disponível para o seu negócio.',
        price: 'Em Breve',
        features: [
...
const PlusIcon: React.FC = () => (
     <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-orange-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const PricingCard: React.FC<{ plan: Plan }> = ({ plan }) => {
    const cardClasses = plan.recommended
        ? 'bg-slate-800 border-2 border-orange-500 transform lg:scale-105'
        : plan.comingSoon ? 'bg-slate-800 opacity-75' : 'bg-slate-800';
    ...
    return (
        <div className={`p-8 rounded-lg shadow-xl relative flex flex-col h-full ${cardClasses}`}>
            {plan.recommended && (
            ...
            <div className="flex-grow flex flex-col">
                ...
                <p className="text-slate-400 text-center mb-6 min-h-[6rem]">{plan.description}</p>
                <div className="text-center mb-6">
                    <span className="text-4xl font-extrabold">{plan.price}</span>
                    {plan.priceDetails && <p className="text-slate-400">{plan.priceDetails}</p>}
                </div>
                ...
                {plan.supportPlan && (
                    <div className="border-t border-slate-700 pt-6 mt-auto">
                        <div className="flex items-center justify-center text-center mb-4">
                            <PlusIcon />
                            <h4 className="text-lg font-bold text-orange-500 ml-2">{plan.supportPlan.title}</h4>
                        </div>
                        <p className="text-2xl font-bold text-center mb-4">{plan.supportPlan.price}</p>
            ...
            <a {...linkProps} className={`w-full mt-8 text-center text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300 ${buttonClasses}`}>
                {plan.buttonText}
            </a>
        </div>
    );
        ...
        <section id="planos" className="py-20 bg-slate-900">
            <div className="container mx-auto px-6">
                <h2 className="text-3xl md:text-4xl font-bold text-center mb-12">
                    Escolha o Plano para Dar a <span className="text-orange-500">Largada</span> no Seu Projeto
                </h2>
