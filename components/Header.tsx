...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="bg-white/90 backdrop-blur-sm sticky top-0 z-50 shadow-sm">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} className="text-slate-700 hover:text-emerald-600 transition-colors duration-300">
                {link.text}
              </a>
            ))}
          </nav>
      ...
      {/* Mobile Menu Panel */}
      <div className={`md:hidden bg-white fixed top-0 right-0 h-full w-4/5 max-w-sm z-50 shadow-2xl transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-end p-4">
            <button onClick={() => setIsMenuOpen(false)} aria-label="Close menu">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-slate-600 hover:text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <nav className="flex flex-col items-start space-y-6 p-6 mt-8">
          {navLinks.map((link) => (
            <a key={link.href} href={link.href} onClick={() => setIsMenuOpen(false)} className="text-xl text-slate-700 hover:text-emerald-600 transition-colors duration-300">
              {link.text}
            </a>
          ))}
        </nav>
